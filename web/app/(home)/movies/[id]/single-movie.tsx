        ...
        <div className="flex rounded-xl flex-row p-5 justify-between align-middle">
          <div>
            {movie.posterUrl && (
              <Image
                src={movie.posterUrl}
                alt={movie.title}
                className="mb-4 rounded-xl sticky top-16"
                width="350"
                height="525"
              />
            )}
          ...
          <div className="ml-2 p-4 h-full w-full">
            ...
            <div className="mb-4">
              ...
              <VoteSlider
                userVote={movie.userVote ? movie.userVote : 5}
                movieId={movie.id}
              />
            </div>
            ...
            <p className="mb-4 text-lg">🕰️ {convertRuntime(movie.runtime)}</p>
            <p className="mb-4 text-lg">📅 {formatDate(movie.releaseDate)}</p>
            <p className="mb-4 text-lg">🎬 {movie.genres.join(", ")}</p>
        ...
        <div className="relative mb-10">
          ...
          <Card className="w-full p-3 bg-white px-20">
            <ul className="grid grid-flow-row w-full">
              {movie.casting.slice(0, 7).map((cast, index) => (
                <li key={index}>
                  <CastingCard cast={cast} />
                </li>
              ))}
