...
export default async function MoviesPage() {
  const recommendedMovies: GetRecommandedMoviesResponse =
    await fetchRecommendedMovies(recommendationLimit);
  ...
  return (
    <QueryProvider>
      <div className="container mx-auto p-4 mt-3">
        ...
        <h2 className="text-2xl font-bold mb-4" id="search">
          Search
        </h2>
        <SearchMovies />
        {recommendedMovies.movies.length > 0 && (
          <>
            <h2 className="text-2xl font-bold mb-4" id="recommended">
              Recommended for you
            </h2>
            <RecommendedMovies movieList={recommendedMovies.movies} />
          </>
        )}
        <h2 className="text-2xl font-bold mb-4 mt-6" id="popular">
          Popular
        </h2>
        <MoviesGrid />
      </div>
