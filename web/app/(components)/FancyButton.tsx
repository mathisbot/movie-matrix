...
export function FancyButton({ href, children, className, type }: { href: string; children: string; className: string; type: "submit" | "reset" | "button" | undefined; }) {
    return (
        <Link href={href} className={className}>
            <button type={ type } className="group cursor-pointer relative cursor-default w-[120px] h-[60px] bg-[linear-gradient(144deg,_#af40ff,_#5b42f3_50%,_#00ddeb)] text-white whitespace-nowrap flex flex-wrap rounded-lg overflow-hidden">
                <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 pointer-events-none">{ children }</span>
